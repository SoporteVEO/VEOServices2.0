...
import { computeAbsenceDays } from "@/components/pages/absences";

type AbsenceImageSrc =
  | string
...
export interface AbsencePdfReportProps {
  absence: Absence;
  /**
   * Optional override for image sources. Defaults to using `absence.images[].url`.
   * Useful in environments (like the API) where the image data should be embedded
   * as buffers instead of being fetched from a URL.
   */
    ...
    >
      ...
      >
        ...
        <Section spacing="md">
          ...
          <Section variant="callout" padding="md" accentColor={theme.colors.info}>
        ...
        <View style={{ marginTop: theme.spacing.sectionGap }}>
          ...
          <Text variant="xs" color="mutedForeground" align="center">
            Este documento es un reporte oficial generado automáticamente por
            el sistema de Recursos Humanos de VEO Services.
          </Text>
        </View>
        ...
        <PageFooter
          ...
          rightText="" // page number rendered separately
          fixed
        />
        <PdfPageNumber
